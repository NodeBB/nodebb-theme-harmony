<nav class="sidebar sidebar-left position-fixed h-100">
	<ul id="main-nav" class="navbar-nav">
		{{{ each navigation }}}
		<!-- IF function.displayMenuItem, @index -->
		<li class="nav-item m-2 {navigation.class}{{{ if navigation.dropdown }}} dropdown{{{ end }}}" title="{navigation.title}">
			<a class="nav-link p-0 rounded-1 navigation-link d-flex justify-content-center align-items-center {{{ if navigation.dropdown }}}dropdown-toggle{{{ end }}}"
